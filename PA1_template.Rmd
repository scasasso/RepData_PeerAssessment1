...
iMax
```
which correspond to the minute `r paste(substr(as.character(steps_vs_int$interval[iMax]),1,1), sep = "", ":",substr(as.character(steps_vs_int$interval[iMax]),nchar(as.character(steps_vs_int$interval[iMax]))-1,nchar(as.character(steps_vs_int$interval[iMax]))))`.
...
nas
```
which account for `r round(100*nas/nrow(df),1)`% of the total observations.
